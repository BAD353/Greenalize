self.onmessage = (e: MessageEvent) => {
  ...
  for (let gy = 0; gy <= gridHeight; gy++) {
    for (let gx = 0; gx <= gridWidth; gx++) {
      ...
      for (const park of parks) {
        ...
        // Score against every outer ring across all polygons of this park
        let minD = Infinity;
        ...
        score +=
          ...
          Math.pow(Math.max(minD, minDist), distPow);
      }
  ...
  for (let y = 0; y < outHeight; y++) {
    ...
    for (let x = 0; x < outWidth; x++) {
      ...
      // Bilinear interpolation over the score grid
      const score =
  ...
  self.postMessage({ imageDataArray, width: outWidth, height: outHeight, bounds: expandedBounds });
};
